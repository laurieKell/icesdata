...
# Helper function from icesdata package
...
if (!is.null(formas_data_dir)) {
  ...
  # Try multiple possible stock data file names
  stock_files <- c(
  ...
  for (stock_file in stock_files) {
    ...
    if (file.exists(stocks_file)) {
      ...
      # Rename ICESStocks to icesdata if needed
...
if (!is.null(formas_data_dir)) {
  ...
  if (file.exists(info_file)) {
    ...
    db_file <- file.path(formas_data_dir, "inputs", "Database.csv")
    if (file.exists(db_file)) {
      info <- read.csv(db_file)[, -9]
      names(info)[1] <- ".id"
    }
  }
}
...
```{r, spp-data}
# Species metadata - create if not already loaded
if (!exists("spp")) {
...
```{r, lw-data}
# Length-weight relationships - create if not already loaded
if (!exists("lw")) {
  lw <- data.frame(
    name = c(
...
```{r, indices}
if (!is.null(formas_data_dir)) {
  idx_file <- file.path(formas_data_dir, "inputs", "Stock_indices.csv")
  if (file.exists(idx_file)) {
    idx <- read.csv(idx_file)[, 1:5]
